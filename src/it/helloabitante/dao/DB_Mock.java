...
public class DB_Mock {
	...
	public static Long getNexIdAvailable() {
		Long resultId = null;
		if (DB_Mock.LISTA_ABITANTI == null || DB_Mock.LISTA_ABITANTI.isEmpty()) {
			resultId = 1L;
		}
		resultId = DB_Mock.LISTA_ABITANTI.stream().max(Comparator.comparing(Abitante::getIdAbitante)).get()
				.getIdAbitante();
		return ++resultId;
	}
}
